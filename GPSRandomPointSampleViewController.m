...
//base map rest url
#define kBaseMapURL @"http://services.arcgisonline.com/ArcGIS/rest/services/ESRI_StreetMap_World_2D/MapServer"
...
@property (nonatomic, strong) AGSSimpleMarkerSymbol *simpleMarkerSymbol;
...
- (IBAction)addCurrentLocationAsVertex
{
    ...
    int i = 0;
    //generate the point near CA
    float latitudeMin=  36.0;
    float latitudeMax = 43.5;
    float longtitudeMin = -122;
    float longtitudeMax = -120;
    //AGSPoint *mapLocation = self.mapView.locationDisplay.mapLocation;
    AGSSimpleMarkerSymbol *markerSymbol = [AGSSimpleMarkerSymbol simpleMarkerSymbol];
    markerSymbol.color = [UIColor yellowColor];
    markerSymbol.outline.color = [UIColor blackColor];
    markerSymbol.size = CGSizeMake(7, 6);
    markerSymbol.style = AGSSimpleMarkerSymbolStyleDiamond;
    
    //symbology to graphic layer
    AGSSimpleRenderer* mySimpleRenderer = [AGSSimpleRenderer simpleRendererWithSymbol:markerSymbol];
    
    self.gpsSketchLayer = [AGSSketchGraphicsLayer graphicsLayer];
    self.gpsSketchLayer.renderer = mySimpleRenderer;
    [self.mapView addMapLayer:self.gpsSketchLayer withName:@"Random Point Layer"];
    
    while (i < 500) {

#define arc4frandom_Max 0x100000000
        float xLong = ((float)arc4random()/arc4frandom_Max) *(longtitudeMax - longtitudeMin) + longtitudeMin;
        float yLat = ((float)arc4random()/arc4frandom_Max) *(latitudeMax - latitudeMin) + latitudeMin;
    
        AGSPoint* point = [AGSPoint pointWithX: xLong y:yLat spatialReference:[AGSSpatialReference spatialReferenceWithWKID:102100]];
        // [self.gpsSketchLayer insertVertex:[self.mapView.locationDisplay mapLocation] inPart:0 atIndex:-1];
    
        AGSGraphic* gr = [AGSGraphic graphicWithGeometry:point symbol:markerSymbol attributes:nil];
        
        [self.gpsSketchLayer addGraphic:gr];
        //[self.gpsSketchLayer insertVertex:mapLocation inPart:0 atIndex:-1];
    ...
    //using this method to add graphics on 10.1.1 version
    //AGSGraphic* gr = [AGSGraphic graphicWithGeometry:point symbol:markerSymbol attributes:nil infoTemplateDelegate:nil];
    
    //using this method to add graphics on 10.2.x version
